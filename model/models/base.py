...
class FewShotModel(nn.Module):
    def __init__(self, args):
        ...
        elif args.backbone_class == 'ConvNet6':
            from model.networks.convnet import ConvNet
            self.encoder = ConvNet(6)
    ...
    def forward(self, xs, get_feature=False):
        if get_feature:
            # get feature with the provided embeddings
            feature=0
            for x in xs:
                feature+=self.encoder(x)
            return feature/len(xs)
        else:
            instance_embs=0
            # feature extraction
            for x in xs:
            ...
            instance_embs/=len(xs)
            num_inst = instance_embs.shape[0]
            # split support query set for few-shot data
            support_idx, query_idx = self.split_instances(x)
            if self.training:
                logits, logits_reg, proto_loss = self._forward(instance_embs, support_idx, query_idx)
                return logits, proto_loss, logits_reg
            else:
                logits, proto_loss = self._forward(instance_embs, support_idx, query_idx)
