...
class Classifier(nn.Module):
    ...
    def forward_proto(self, data_shot, data_query, way = None):
        ...
        kappa=torch.tensor(self.args.kappa,dtype=torch.float)
        logits_sim=torch.div(2*(torch.exp(kappa*logits_sim)-torch.exp(-kappa)),torch.exp(kappa)-torch.exp(-kappa)) - 1.
